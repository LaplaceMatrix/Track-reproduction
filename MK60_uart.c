...
/************cc协议*************/
#define RocketDataChannel 6    //通道数量
uint8 RocketSendBuf[RocketDataChannel*2]={0};
uint8 RocketDataNum= RocketDataChannel*2;
/***************************Rocket*****************************************/
...
void sendDataToRocket(void)
{
  ...
  uart_putchar(UART0,0x52);
  uart_putchar(UART0,0xcc);
  for(i=0;i<RocketDataNum;i++)
  {
    uart_putchar(UART0,RocketSendBuf[i]);

  }
  uart_putchar(UART0,0x88);
}

void Rocketpush(uint8 ch,uint16 data)
{
    RocketSendBuf[ch*2]=data/256;
    RocketSendBuf[ch*2+1]=data%256;
}
